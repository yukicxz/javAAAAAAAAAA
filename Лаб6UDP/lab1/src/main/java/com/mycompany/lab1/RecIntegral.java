...
public class RecIntegral implements Serializable {
    ...
    for (int i = 0; i < threadCount; i++) {
        int stepsForThisThread = stepsPerThread + (i < remainder ? 1 : 0);

        double a = lowerLimit + currentStep * step;
        double b = a + stepsForThisThread * step;

        tasks[i] = new IntegralPotok(a, b, step);
        threads[i] = new Thread(tasks[i]);
        threads[i].start();

        currentStep += stepsForThisThread;
    }
